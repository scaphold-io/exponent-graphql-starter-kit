...
const client = new ApolloClient({
  networkInterface: createNetworkInterface({
    uri: 'https://us-west-2.api.scaphold.io/graphql/exponent-starter-kit'
  }),
});
...
class App extends React.Component {
  render() {
    ...
    let messages = this.props.data.viewer.allMessages.edges;
    ...
    return (
      <View style={styles.container}>
        <ScrollView
          ...
          {messages.map((message, i) => <Text key={i}>{message.node.text}</Text>)}
        </ScrollView>
...
@graphql(gql`
  mutation AddMessage($msg: CreateMessageInput!) {
